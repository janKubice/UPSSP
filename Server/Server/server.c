...
void *connection_handler(void *socket_desc)
{
    ...
    while( (read_size = recv(sock , client_message , CLIENT_MSG_SIZE , 0)) > 0 )
    {
            ...
            for (g = 0; g < NUMBER_OF_ROOMS;g++){
                for (t = 0; t < NUMBER_PLAYERS_IN_ONE_GAME; t++) {
                    if(games[g].availability[t] == 1) {
                        if(players[games[game_id].array_p[t].id_p].id_p == player_id) {
                        	game_id = g;
                        	break;
                        }
                    }
        ...
        //opuštění hry
            ...
            if (is_admin == 1){
                games[game_id].array_p[0].end_game = 0;
                ...
                for (p = 1; p < 3; p++){
                    if (games[game_id].availability[p] == 1){
                        games[game_id].array_p[p].end_game = 0;
                        write(games[game_id].array_p[p].socket, msg, strlen(msg));
                    }       
                }
                free(msg);
            ...
            if (rec == 0){
                for(i = 1; i < NUMBER_OF_PLAYERS; i++){
                    if(players[i].id_p == -1){
                        ...
                        players[i].points = 0;
                        memcpy(players[i].name, params[2], strlen(params[2]));
                        break;
                    }
                }
